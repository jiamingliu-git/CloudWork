...
Headers = {'Content-Type': 'application/json;charset=UTF-8'}           #获取{'Content-Type': 'application/json;charset=UTF-8'}
...
# 登录
    def setUp(self):
        ...
        url=cf['login']['url']
        data=cf['login']['data']
        headers = cf['headers']['headers']

        login_res=TestMethod_Box_01().login('post',url=eval(url),data=eval(data),headers=eval(headers))
        ...
        try:
            ...
            raise e
#获取登录后token
        # headers['Authorization'] = login_res.json()['value']
